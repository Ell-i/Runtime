...
void 
ENC28J60Class::begin(const uint8_t mac_address[ETH_ADDRESS_LEN]) const {
...
ENC28J60Class::checkBegin(const uint8_t mac_address[ETH_ADDRESS_LEN]) const {
    ...
    if(enc_reg_get(MAC_ADR0) != mac_address[5]) return false;
    if(enc_reg_get(MAC_ADR1) != mac_address[4]) return false;
    if(enc_reg_get(MAC_ADR2) != mac_address[3]) return false;
    if(enc_reg_get(MAC_ADR3) != mac_address[2]) return false;
    if(enc_reg_get(MAC_ADR4) != mac_address[1]) return false;
    if(enc_reg_get(MAC_ADR5) != mac_address[0]) return false;

    for (p = enc28j60_init + 14;                          //skip buffer pointers
         p < enc28j60_init + COUNT_OF(enc28j60_init) - 1; //skip ECON1
         p++) {
        if (enc_reg_get(p->reg) != p->value) return false;
    }
